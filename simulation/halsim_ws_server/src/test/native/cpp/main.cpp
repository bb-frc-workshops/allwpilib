...
TEST_F(WebServerIntegrationTest, DigitalOutput) {
  ...
  timer->timeout.connect([EXPECTED_VALUE, PIN] { 
    wpi::outs() << "***** Setting DIO value for pin "<< PIN << " to " << (EXPECTED_VALUE ? "true" : "false") << "\n";
...
TEST_F(WebServerIntegrationTest, DigitalInput) {
  ...
  timer->timeout.connect([EXPECTED_VALUE, PIN] {      
    wpi::json msg = {
      {"type", "DIO"}, {"device", std::to_string(PIN)}, {"data", {{"<>value", EXPECTED_VALUE}}}
    };
