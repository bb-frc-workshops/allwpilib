...
void HALSimWSProviderJoystick::RegisterCallbacks() {
  m_axesCbKey = HALSIM_RegisterJoystickAxesCallback(m_channel,
      [](const char* name, void* param, int32_t joystickNum, const HAL_JoystickAxes* axes) {
  ...
  m_povsCbKey = HALSIM_RegisterJoystickPOVsCallback(m_channel,
      [](const char* name, void* param, int32_t joystickNum, const HAL_JoystickPOVs* povs) {
        std::vector<int16_t> povsValues;
        for (int16_t i = 0; i < povs->count; i++) {
  ...
  m_buttonsCbKey = HALSIM_RegisterJoystickButtonsCallback(m_channel,
      [](const char* name, void* param, int32_t joystickNum, const HAL_JoystickButtons* buttons) {
        std::vector<bool> buttonsValues;
        for (uint8_t i = buttons->count; i >= 0; i--) {
