...
void HALSimWSProviderEncoder::RegisterCallbacks() {
  m_initCbKey = HALSIM_RegisterEncoderInitializedCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderEncoder*>(param)->ProcessHalCallback(
        {{"<init", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_countCbKey = HALSIM_RegisterEncoderCountCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderEncoder*>(param)->ProcessHalCallback(
        {{">count", value->data.v_int}});
  }, this, true);

  m_periodCbKey = HALSIM_RegisterEncoderPeriodCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderEncoder*>(param)->ProcessHalCallback(
        {{">period", value->data.v_double}});
  }, this, true);

  m_resetCbKey = HALSIM_RegisterEncoderResetCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderEncoder*>(param)->ProcessHalCallback(
        {{"<reset", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_reverseDirectionCbKey = HALSIM_RegisterEncoderReverseDirectionCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderEncoder*>(param)->ProcessHalCallback(
        {{"<reverse_direction", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_samplesCbKey = HALSIM_RegisterEncoderSamplesToAverageCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderEncoder*>(param)->ProcessHalCallback(
...
void HALSimWSProviderEncoder::CancelCallbacks() {
  ...
  HALSIM_CancelEncoderReverseDirectionCallback(m_channel, m_reverseDirectionCbKey);
