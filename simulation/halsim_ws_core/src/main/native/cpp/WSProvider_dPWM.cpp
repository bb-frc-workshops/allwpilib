...
void HALSimWSProviderDigitalPWM::RegisterCallbacks() {
  m_initCbKey = HALSIM_RegisterDigitalPWMInitializedCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDigitalPWM*>(param)->ProcessHalCallback(
        {{"<init", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_dutyCycleCbKey = HALSIM_RegisterDigitalPWMDutyCycleCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDigitalPWM*>(param)->ProcessHalCallback(
        {{"<duty_cycle", value->data.v_double}});
  }, this, true);

  m_pinCbKey = HALSIM_RegisterDigitalPWMPinCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDigitalPWM*>(param)->ProcessHalCallback(
