...
#define REGISTER_AIN_ACCUM(halsim, jsonid, ctype, haltype) \
  HALSIM_RegisterAnalogInAccumulator##halsim##Callback( \
      ...
      [](const char* name, void* param, const struct HAL_Value* value) { \
        static_cast<HALSimWSProviderAnalogIn*>(param)->ProcessHalCallback( \
            {{"accum", {{jsonid, static_cast<ctype>(value->data.v_##haltype)}}}}); \
      }, \
      this, true)
...
void HALSimWSProviderAnalogIn::RegisterCallbacks() {
  ...
  m_oversampleCbKey = REGISTER_AIN(OversampleBits, "<oversample_bits", int32_t, int);
  ...
  m_accumValueCbKey = REGISTER_AIN_ACCUM(Value, ">value", int64_t, long);
  m_accumCountCbKey = REGISTER_AIN_ACCUM(Count, ">count", int64_t, long);
  m_accumCenterCbKey = REGISTER_AIN_ACCUM(Center, "<center", int32_t, int);
  m_accumDeadbandCbKey = REGISTER_AIN_ACCUM(Deadband, "<deadband", int32_t, int);
}
