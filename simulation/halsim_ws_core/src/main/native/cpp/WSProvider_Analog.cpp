...
void HALSimWSProviderAnalogIn::RegisterCallbacks() {
  m_initCbKey = HALSIM_RegisterAnalogInInitializedCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderAnalogIn*>(param)->ProcessHalCallback(
        {{"<init", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_avgbitsCbKey = HALSIM_RegisterAnalogInAverageBitsCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderAnalogIn*>(param)->ProcessHalCallback(
        {{"<avg_bits", value->data.v_int}});
  }, this, true);

  m_oversampleCbKey = HALSIM_RegisterAnalogInOversampleBitsCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderAnalogIn*>(param)->ProcessHalCallback(
        {{"<oversample_bits", value->data.v_int}});
  }, this, true);

  m_voltageCbKey = HALSIM_RegisterAnalogInVoltageCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderAnalogIn*>(param)->ProcessHalCallback(
        {{">voltage", value->data.v_double}});
  }, this, true);

  m_accumInitCbKey = HALSIM_RegisterAnalogInAccumulatorInitializedCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderAnalogIn*>(param)->ProcessHalCallback(
        {{"accum", {{"<init", static_cast<bool>(value->data.v_boolean)}}}});
  }, this, true);

  m_accumValueCbKey = HALSIM_RegisterAnalogInAccumulatorValueCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderAnalogIn*>(param)->ProcessHalCallback(
        {{"accum", {{">value", value->data.v_long}}}});
  }, this, true);

  m_accumCountCbKey = HALSIM_RegisterAnalogInAccumulatorCountCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderAnalogIn*>(param)->ProcessHalCallback(
        {{"accum", {{">count", value->data.v_long}}}});
  }, this, true);

  m_accumCenterCbKey = HALSIM_RegisterAnalogInAccumulatorCenterCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderAnalogIn*>(param)->ProcessHalCallback(
        {{"accum", {{"<center", value->data.v_int}}}});
  }, this, true);

  m_accumDeadbandCbKey = HALSIM_RegisterAnalogInAccumulatorDeadbandCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderAnalogIn*>(param)->ProcessHalCallback(
...
void HALSimWSProviderAnalogIn::CancelCallbacks() {
  ...
  HALSIM_CancelAnalogInAccumulatorInitializedCallback(m_channel, m_accumInitCbKey);
  ...
  HALSIM_CancelAnalogInAccumulatorDeadbandCallback(m_channel, m_accumDeadbandCbKey);
}
...
void HALSimWSProviderAnalogOut::RegisterCallbacks() {
  m_initCbKey = HALSIM_RegisterAnalogOutInitializedCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderAnalogOut*>(param)->ProcessHalCallback(
        {{"<init", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_voltageCbKey = HALSIM_RegisterAnalogOutVoltageCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderAnalogOut*>(param)->ProcessHalCallback(
        {{"<voltage", value->data.v_double}});
  }, this, true);
}
