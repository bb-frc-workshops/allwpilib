...
void HALSimWSProviderRelay::RegisterCallbacks() {
  m_initFwdCbKey = HALSIM_RegisterRelayInitializedForwardCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderRelay*>(param)->ProcessHalCallback(
        {{"<init_fwd", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_initRevCbKey = HALSIM_RegisterRelayInitializedReverseCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderRelay*>(param)->ProcessHalCallback(
        {{"<init_rev", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_fwdCbKey = HALSIM_RegisterRelayForwardCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderRelay*>(param)->ProcessHalCallback(
        {{"<fwd", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_revCbKey = HALSIM_RegisterRelayReverseCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderRelay*>(param)->ProcessHalCallback(
