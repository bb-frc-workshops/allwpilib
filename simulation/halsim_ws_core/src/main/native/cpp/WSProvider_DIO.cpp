...
void HALSimWSProviderDIO::RegisterCallbacks() {
  m_initCbKey = HALSIM_RegisterDIOInitializedCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDIO*>(param)->ProcessHalCallback(
        {{"<init", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_valueCbKey = HALSIM_RegisterDIOValueCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDIO*>(param)->ProcessHalCallback(
        {{"<>value", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_pulseLengthCbKey = HALSIM_RegisterDIOPulseLengthCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDIO*>(param)->ProcessHalCallback(
        {{"<pulse_length", value->data.v_double}});
  }, this, true);

  m_inputCbKey = HALSIM_RegisterDIOIsInputCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDIO*>(param)->ProcessHalCallback(
