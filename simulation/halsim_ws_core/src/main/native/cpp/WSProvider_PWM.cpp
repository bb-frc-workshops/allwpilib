...
void HALSimWSProviderPWM::RegisterCallbacks() {
  m_initCbKey = HALSIM_RegisterPWMInitializedCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderPWM*>(param)->ProcessHalCallback(
        {{"<init", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_speedCbKey = HALSIM_RegisterPWMSpeedCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderPWM*>(param)->ProcessHalCallback(
        {{"<speed", value->data.v_double}});
  }, this, true);

  m_positionCbKey = HALSIM_RegisterPWMPositionCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderPWM*>(param)->ProcessHalCallback(
        {{"<position", value->data.v_double}});
  }, this, true);

  m_rawCbKey = HALSIM_RegisterPWMRawValueCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderPWM*>(param)->ProcessHalCallback(
        {{"<raw", value->data.v_int}});
  }, this, true);

  m_periodScaleCbKey = HALSIM_RegisterPWMPeriodScaleCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderPWM*>(param)->ProcessHalCallback(
        {{"<period_scale", value->data.v_int}});
  }, this, true);

  m_zeroLatchCbKey = HALSIM_RegisterPWMZeroLatchCallback(m_channel, [](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderPWM*>(param)->ProcessHalCallback(
