...
void HALSimWSProviderDriverStation::Initialize(WSRegisterFunc webRegisterFunc) {
  ...
  CreateSingleProvider<HALSimWSProviderDriverStation>(
      "DriverStation", webRegisterFunc);
}

void HALSimWSProviderDriverStation::RegisterCallbacks() {
  m_enabledCbKey = HALSIM_RegisterDriverStationEnabledCallback([](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDriverStation*>(param)->ProcessHalCallback(
        {{">enabled", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_autonomousCbKey = HALSIM_RegisterDriverStationAutonomousCallback([](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDriverStation*>(param)->ProcessHalCallback(
        {{">autonomous", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_testCbKey = HALSIM_RegisterDriverStationTestCallback([](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDriverStation*>(param)->ProcessHalCallback(
        {{">test", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_estopCbKey = HALSIM_RegisterDriverStationEStopCallback([](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDriverStation*>(param)->ProcessHalCallback(
        {{">estop", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_fmsCbKey = HALSIM_RegisterDriverStationFmsAttachedCallback([](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDriverStation*>(param)->ProcessHalCallback(
        {{">fms", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_dsCbKey = HALSIM_RegisterDriverStationDsAttachedCallback([](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDriverStation*>(param)->ProcessHalCallback(
        {{">ds", static_cast<bool>(value->data.v_boolean)}});
  }, this, true);

  m_allianceCbKey = HALSIM_RegisterDriverStationAllianceStationIdCallback([](const char* name, void* param, const struct HAL_Value* value) {
    std::string station;
  ...
  m_matchTimeCbKey = HALSIM_RegisterDriverStationMatchTimeCallback([](const char* name, void* param, const struct HAL_Value* value) {
    static_cast<HALSimWSProviderDriverStation*>(param)->ProcessHalCallback(
